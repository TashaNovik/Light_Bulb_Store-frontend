...
@router.get("/users", response_model=schemas.PaginatedAdminUserResponse)
async def get_users(
    request: Request,
    ...
    if search:
        ...
        query = query.filter(
            (models.AdminUser.username.ilike(search_filter)) |
            (models.AdminUser.email.ilike(search_filter)) |
            (models.AdminUser.first_name.ilike(search_filter)) |
            (models.AdminUser.last_name.ilike(search_filter))
        )
    
    # Apply active status filter
    if isActive is not None:
        query = query.filter(models.AdminUser.is_active == isActive)
